...
parser.add_argument('--image_text_folder', type=str, required=False,
...
train_group.add_argument('--batch_size', default = 5, type = int, help = 'Batch size')
...
model_group.add_argument('--dim', default = 312, type = int, help = 'Model dimension')

model_group.add_argument('--text_seq_len', default =512, type = int, help = 'Text sequence length') #256

model_group.add_argument('--depth', default =16, type = int, help = 'Model depth')

model_group.add_argument('--heads', default = 12, type = int, help = 'Model number of heads')
...
DALLE_OUTPUT_FILE_NAME = params['dalle_output_file_name'] + ".pt"
...
#MODEL_DIM = args.dim
MODEL_DIM = params['model_dimension']

#TEXT_SEQ_LEN = args.text_seq_len
TEXT_SEQ_LEN = params['text_sequence_length']
...
DIM_HEAD = params['head_dimension']
...
###############################
IMAGE_TEXT_DATASET = params['dataset']

if not ENABLE_WEBDATASET:
    # quit early if you used the wrong folder name
    assert Path(IMAGE_TEXT_DATASET).exists(), f'The path {IMAGE_TEXT_DATASET} was not found.' ###################################
else:
    # quit early if no tar files were found
    if Path(IMAGE_TEXT_DATASET).is_dir():###################
        DATASET = [str(p) for p in Path(IMAGE_TEXT_DATASET).glob("**/*") if ".tar" in str(p).lower()] # .name   ######################
        assert len(DATASET) > 0, 'The directory ({}) does not contain any WebDataset/.tar files.'.format(IMAGE_TEXT_DATASET) ####################
        print('Found {} WebDataset .tar(.gz) file(s) under given path {}!'.format(len(DATASET), IMAGE_TEXT_DATASET))  ###################
    elif ('http://' in IMAGE_TEXT_DATASET.lower()) | ('https://' in IMAGE_TEXT_DATASET.lower()):   ###################
        DATASET = f"pipe:curl -L -s {IMAGE_TEXT_DATASET} || true"  ###############
        print('Found {} http(s) link under given path!'.format(len(DATASET), IMAGE_TEXT_DATASET)) ###############
    elif 'gs://' in IMAGE_TEXT_DATASET.lower():      ###########
        DATASET = f"pipe:gsutil cat {IMAGE_TEXT_DATASET} || true"  #########
        print('Found {} GCS link under given path!'.format(len(DATASET), IMAGE_TEXT_DATASET))  #############
    elif '.tar' in IMAGE_TEXT_DATASET:  ##################
        DATASET = IMAGE_TEXT_DATASET    #################
        print('Found WebDataset .tar(.gz) file under given path {}!'.format(IMAGE_TEXT_DATASET)) #########
    else:
        raise Exception('No folder, no .tar(.gz) and no url pointing to tar files provided under {}.'.format(IMAGE_TEXT_DATASET))  ############
...
#model = dalle
#paral_model = torch.nn.DataParallel(model, device_ids=[0, 1])    ###nn.DataParallel(model,device_ids = [0, 1])
#paral_model.to(device)

model = dalle.to(device)
